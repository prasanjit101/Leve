...
def test_streamed_tokens_suppress_duplicate_message():
    n = EventNormalizer()
    events = [
        {"event": "on_chat_model_stream", "run_id": "r1", "data": {"chunk": _chunk("Hel")}},
        {"event": "on_chat_model_stream", "run_id": "r1", "data": {"chunk": _chunk("lo")}},
        {"event": "on_chat_model_end", "run_id": "r1", "data": {"output": _chunk("Hello")}},
    ]
...
def test_empty_message_dropped():
    n = EventNormalizer()
    assert n.normalize(
        {"event": "on_chat_model_end", "run_id": "r1", "data": {"output": _chunk("")}}
    ) is None


def test_separate_runs_are_independent():
    n = EventNormalizer()
    n.normalize({"event": "on_chat_model_stream", "run_id": "a", "data": {"chunk": _chunk("x")}})
    # A different run that did not stream still gets its message.
    out = n.normalize(
        {"event": "on_chat_model_end", "run_id": "b", "data": {"output": _chunk("done")}}
    )
